...
redisClient.on('ready', () => {
    ...
    redisClient.set(REDIS_KEY, JSON.stringify(payload), (err, res) => {
        if (err) {
...
const getUser = (userID) => {
    return axios_1.default.get(`${SLACK_URLS.GET_USER}?user=${userID}`, {
...
const handleViewSubmission = (payload) => {
    ...
    getUser(userID)
        ...
        newDocRef
            .set(issue)
            .then(() => {
            postMessage(issue.message.channel.id, issue.message.ts, "An issue has been created in response to this message. The engineer-on-call will look into it ASAP!")
                .then(() => console.log("posted message successfully"))
                .catch((err) => console.log("failed to post message:", err));
        })
            .catch((err) => {
